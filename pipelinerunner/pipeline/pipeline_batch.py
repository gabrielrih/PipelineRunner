...
class PipelineBatchExecutor:
    def __init__(self, runners: List[RunnerModel], mode: PipelineExecutionMode, dry_run: bool = False):
    ...
    def run_all(self):
        if self.mode == PipelineExecutionMode.SEQUENTIAL:
            for runner in self.runners:
                SequentialPipelineExecutor(runner, self.dry_run).run()
            return
        
        for runner in self.runners:
            ParallelPipelineExecutor(runner, self.dry_run).run()
